...
class ToFSensor:
    def __init__(self):
        # Look for the library in the same directory as this Python module
        lib_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "libtof.so")
        if not os.path.exists(lib_path):
            raise FileNotFoundError(f"VL53L1 library not found at {lib_path}. Please ensure the package is properly installed.")
